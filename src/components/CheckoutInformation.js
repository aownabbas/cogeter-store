...
function CheckoutInformation({ hide, proceedToConfirmation, cartItems }) {
  ...
  const onCheckOutClick = async () => {
    ...
    items = cartItems?.map((item) => ({
      ...
      price:
        (item.onSale ? item.salePrice : item.regularPrice) * rateMultiplier, // Replace with the actual price
    }));
    let payload = {
      data: {
        ...
        delivery_charges: standardShipping
          ? standardDeliveryCharges
          : sameDayDeliveryCharges,
        address: { ...values.address, country: _selectedCountry.id },
        items: items,
        user: userObject,
      },
    };
    ...
    try {
      if (cartItems?.length === 0) {
        ...
        toast.warning("Select or add a new address");
        return;
      }
      ...
      if (isTabbySelect) {
        ...
        if (tabbyResponse.status !== 200) {
          setisLoading(false);
          toast.warning(
            tabbyResponse.data.errorMessage ?? "Tabby  not available"
          );
          return;
        }
      }
      ...
      <div className="container">
        <div className="row gx-5 ">
          ...
          <div className="col-md-6">
                          ...
                          <div className="d-flex align-items-center">
                            ...
                            <div>
                              ...
                              <p className="mb-0 ms-4 shipping_subtitle">
                                ...
                                in Sharjah, Ajman, Dubai, and Abu Dhabi).
                              </p>
                            </div>
                          </div>
                          <div
            ...
            <div className={`card_expand  ${isExpanded ? "expanded" : ""}`}>
